...
/* La interfaz Mediator declara un método utilizado por los componentes para notificar
al mediador sobre varios eventos. El Mediador puede reaccionar a estos eventos y
pasar la ejecucion a otros componentes.*/
class BaseComponent;
class Mediator{
	public:
		virtual void Notify(BaseComponent *remitente, string event) const = 0;
};

/* El componente base proporciona la funcionalidad básica de almacenar la instancia
de un mediador dentro de los objetos del componente.*/
class BaseComponent{
 	protected:
  		Mediator *mediator_;
 	public:
...
Tampoco dependen de ninguna clase mediadora concreta.*/
...
void ClientCode() {
  	...
  	c1->DoA();
  	cout<<"\nEl cliente activa la operacion D.\n";
  	c2->DoD();
